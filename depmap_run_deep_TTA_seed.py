...
from depmap_rna_model import Model
import pandas as pd 
import os
...
depmap_input_path = "./coderdata_input/depmap_transcriptomics.csv.gz"
...
def run_experiment(data_split_seed):
    ...
    if not os.path.exists("shared_input/depmap_transcriptomics_wide.tsv"):
        DataProcessor.convert_long_to_wide_format(depmap_input_path)
    depmap_gene_exp = pd.read_csv("shared_input/depmap_transcriptomics_wide.tsv", sep='\t')

    mpnst_gene_exp, depmap_gene_exp = intersect_columns(mpnst_gene_exp, depmap_gene_exp, selected_gene_df)
...
results_file_path = f'seed_{data_split_seed}_epoch_100_depmap_rna_train_results_table.txt'
