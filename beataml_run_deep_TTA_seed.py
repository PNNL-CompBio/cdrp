...
# def intersect_columns(mpnst_gene_exp, beataml_gene_exp, selected_gene_df):
#     selected_genes = selected_gene_df.iloc[:, 1].tolist()
#     common_columns = list("improve_sample_id",set(selected_genes).intersection(set(mpnst_gene_exp.columns), set(beataml_gene_exp.columns)))
...
def intersect_columns(mpnst_gene_exp, beataml_gene_exp, selected_gene_df):
    ...
    common_columns = list(set(selected_genes_str).intersection(set(mpnst_gene_exp.columns), set(beataml_gene_exp.columns)))
    # Ensure "improve_sample_id" is included if it exists in both DataFrames
    if "improve_sample_id" in mpnst_gene_exp.columns and "improve_sample_id" in beataml_gene_exp.columns:
    ...
    beataml_gene_exp_filtered = beataml_gene_exp[common_columns]
    return mpnst_gene_exp_filtered, beataml_gene_exp_filtered
...
def run_experiment(data_split_seed):
    ...
    mpnst_gene_exp, beataml_gene_exp = intersect_columns(mpnst_gene_exp, beataml_gene_exp, selected_gene_df)
    ...
    model = Model(gnn_features = None, encoder_type='transformer',n_genes=len(mpnst_gene_exp.columns)).to(device)
    # lr = 1e-4
...
results_file_path = f'seed_{data_split_seed}_epoch_100_beataml_rna_train_results_table.txt'
