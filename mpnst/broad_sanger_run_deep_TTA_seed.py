...
from broad_rna_model import Model
import pandas as pd 
import os
...
broad_input_path = "./coderdata_input/broad_transcriptomics.csv.gz"
...
# def intersect_columns(mpnst_gene_exp, broad_gene_exp, selected_gene_df):
#     selected_genes = selected_gene_df.iloc[:, 1].tolist()
#     common_columns = list("improve_sample_id",set(selected_genes).intersection(set(mpnst_gene_exp.columns), set(broad_gene_exp.columns)))
#     mpnst_gene_exp = mpnst_gene_exp[common_columns]
#     broad_gene_exp = broad_gene_exp[common_columns]
#     return mpnst_gene_exp, broad_gene_exp
def intersect_columns(mpnst_gene_exp, broad_gene_exp, selected_gene_df):
    ...
    common_columns = list(set(selected_genes_str).intersection(set(mpnst_gene_exp.columns), set(broad_gene_exp.columns)))
    # Ensure "improve_sample_id" is included if it exists in both DataFrames
    if "improve_sample_id" in mpnst_gene_exp.columns and "improve_sample_id" in broad_gene_exp.columns:
    ...
    broad_gene_exp_filtered = broad_gene_exp[common_columns]
    return mpnst_gene_exp_filtered, broad_gene_exp_filtered
...
def run_experiment(data_split_seed):
    ...
    if not os.path.exists("shared_input/broad_transcriptomics.csv.gz_wide.tsv"):
        DataProcessor.convert_long_to_wide_format(broad_input_path)
    broad_gene_exp = pd.read_csv("shared_input/broad_transcriptomics_wide.tsv", sep='\t')

    mpnst_gene_exp, broad_gene_exp = intersect_columns(mpnst_gene_exp, broad_gene_exp, selected_gene_df)
    ...
    # broad: process experiment & drug data
    broad_exp = pd.read_csv("./coderdata_input/broad_experiments.csv.gz", compression='gzip')
    broad_drugs = pd.read_csv("./coderdata_input/broad_drugs.tsv.gz", sep='\t', compression='gzip')
    # average auc values for the same improve_sample_id and drug_id
    broad_exp = average_auc(broad_exp)
    # add smiles and split data
    broad_df_all = add_smiles(broad_drugs, broad_exp, "auc")
    ...
    broad_common_ids = set(broad_df_all['improve_sample_id'].unique()).intersection(set(broad_gene_exp['improve_sample_id'].unique()))
    # Filter RNA & drug to only include rows with improve_sample_id in the intersection
    broad_df = broad_df_all[broad_df_all['improve_sample_id'].isin(broad_common_ids)].reset_index(drop=True)
    broad_gene_exp = broad_gene_exp[broad_gene_exp['improve_sample_id'].isin(broad_common_ids)]
    train, val= split_df(df=broad_df, seed=data_split_seed)
    ...
    broad_gene_exp = broad_gene_exp.set_index('improve_sample_id')
    # Now perform the scaling operation on the DataFrame without the index column
    scaler = StandardScaler() # mean=0, unit variance
    broad_gene_exp_scaled = scaler.fit_transform(broad_gene_exp)
    # When creating the new DataFrame, use the same columns as the gene_exp DataFrame
    # Because gene_exp now does not include 'improve_sample_id' column, we don't need to adjust column names
    broad_gene_exp_scaled = pd.DataFrame(broad_gene_exp_scaled, index=broad_gene_exp.index, columns=broad_gene_exp.columns)
    data_creater = CreateData(gexp=broad_gene_exp_scaled, encoder_type='transformer', metric="auc", data_path= "shared_input/")
...
results_file_path = f'seed_{data_split_seed}_epoch_100_broad_rna_train_results_table.txt'
