...
#     # Placeholder function to calculate PCC, SRCC, and sample size
#     pcc = np.corrcoef(true_values, predicted_values)[0, 1]
#     srcc = pd.Series(true_values).corr(pd.Series(predicted_values), method='spearman')
...
def plot_results(true_values, predicted_values, ckpt_path, test_df):
    ...
    plt.savefig(f'plots/zzz_{ckpt_basename}_true_vs_predicted.pdf')
...
def run_experiment(data_split_seed, bs, lr, n_epochs,
    ...
    test_rmse, pearson_corr, spearman_corr, true_values, predicted_values= test_fn(test_loader, model, device)
    ...
    plot_df = pd.DataFrame({
        'True fit_AUC': true_values,
        'Predicted fit_AUC': predicted_values,
        'improve_sample_id': test_df_all.loc[test_df_all['improve_sample_id'].isin(test_common_ids), 'improve_sample_id'],
        'improve_drug_id': test_df_all.loc[test_df_all['improve_sample_id'].isin(test_common_ids), 'improve_drug_id']
    })

    # plt.figure(figsize=(10, 6))
    # sns.scatterplot(data=plot_df, x='True fit_AUC', y='Predicted fit_AUC', hue='improve_sample_id', style='improve_drug_id', palette='deep')
    # plt.plot([0, 1], [0, 1], 'r--')
    # plt.title(f'Comparison of True and Predicted fit_AUC\nCheckpoint: {ckpt_path}\nPCC: {pearson_corr:.3f}, SRCC: {spearman_corr:.3f}, Sample Size: {len(plot_df)}')
    # plt.xlabel('True fit_AUC')
    # plt.ylabel('Predicted fit_AUC')
    # plt.grid(True)
    # plt.legend(title='improve_sample_id & improve_drug_id', bbox_to_anchor=(1.05, 1), loc='upper left')
    # plt.tight_layout()
    # Create the scatter plot
    plt.figure(figsize=(10, 6))

    # Scatter plot with different colors for each improve_sample_id
    # for sample_id in plot_df['improve_sample_id'].unique():
    #     subset = plot_df[plot_df['improve_sample_id'] == sample_id]
    #     plt.scatter(subset['True fit_AUC'], subset['Predicted fit_AUC'], label=sample_id)
    ...
    plt.grid(True)
    plt.legend(title='improve_sample_id', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()

    plt.savefig(f'plots/zzz_seed_{data_split_seed}_epoch_{n_epochs}_070924_comparison_true_predicted_fit_AUC_colored.png')
    # plt.show()
    return test_rmse, pearson_corr, spearman_corr

def main():
    parser = argparse.ArgumentParser(description='Deep TTA RNA Model')
    parser.add_argument('--data_split_seed', type=int, default=1, help='Seed for data split')
    ...
    parser.add_argument('--checkpoint_path', type=str, default='/people/moon515/mpnst_smile_model/models/BeatAML_MPNST_proteomics_dss_model_seed_10_epoch_99.pt', help='Path to the model checkpoint to evaluate')
